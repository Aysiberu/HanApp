...
// Basic numeric validation for mobile & zip (only digits allowed)
if (!empty($mobile) && !preg_match('/^[0-9]+$/', $mobile)) {
    echo "<script>alert('Mobile number must contain only digits.'); window.location.href='signuppage.php';</script>";
    exit;
}
if (!empty($zip) && !preg_match('/^[0-9]+$/', $zip)) {
    echo "<script>alert('ZIP code must contain only digits.'); window.location.href='signuppage.php';</script>";
    exit;
}

// validate location against allowed Ilocos Norte list
$allowed = ['Adams','Bacarra','Badoc','Bangui','Banna','Burgos','Carasi','Currimao','Dingras','Dumalneg','Laoag','Marcos','Nueva Era','Pagudpud','Piddig','Pinili','San Nicolas','Sarrat','Solsona','Vintar'];
if (!empty($location) && !in_array($location, $allowed)) {
    echo "<script>alert('Please select a valid location from Ilocos Norte.'); window.location.href='signuppage.php';</script>";
    exit;
}
